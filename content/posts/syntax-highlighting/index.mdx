...
images: 'images/thumbnails/text-highlight.jpeg'
imageAuthorID: '@esteejanssens'
imageAuthor: 'Estée Janssens'
keywords: ['syntax highlighting, prism, MDX syntax highlight']
---

In this post we will be looking into how to enable syntax highlighting for Gatsby MDX files using [prism-react-renderer](https://github.com/FormidableLabs/prism-react-renderer)
...
Then, we create the component which we will use in the `<MDXProvider/>` that is going to provider a wrapper replacement for our application.
...
To render the code block we added in MDX file to show up on the page we will access props of the child component of `<pre/>` component as markdown code blocks gets compiled into a `<pre/>` component wrapping the code component.
...
To get the language defined in the code component in MDX file, we will drill down at `props.children.props.className`, if there is no class name then we might have not put a language on our code block in which case we will default to empty strings.
